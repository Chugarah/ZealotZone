    ...
    where TEntity : class
{
    ...
    public virtual async Task<RepositoryResult<TDomain?>> CreateAsync(TDomain? domainEntity)
    {
        ...
        try
        {
            ...
            if (entity is null)
            {
                return resultFactory.OperationFailed<TDomain?>(
                    new Error("Repository.OperationFailed",
                        $"Failed to convert domain entity to entity: {domainEntity}"), 500);
            }
            ...
            if (createdDomainEntity is null)
            {
                return resultFactory.OperationFailed<TDomain?>(
                    new Error("Repository.OperationFailed",
                        $"Failed to convert entity to domain entity: {entity}"), 500);
            }
        ...
        {
            // Log the exception
            return resultFactory.OperationFailed<TDomain?>(new Error("Repository.OperationFailed", ex.Message), 500);
        }
    }
